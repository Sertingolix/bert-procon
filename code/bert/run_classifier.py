...
flags.DEFINE_bool("do_eval_train", False, "Whether to run eval on the train set.")
...
flags.DEFINE_bool(
    "do_predict_train", False,
    "Whether to run the model in inference mode on the train set.")
...
class FNC1Processor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    traindata = os.path.join(data_dir,"train.tsv")
    print(f'trainfile: {traindata}')
    return self._create_examples(
        self._read_tsv(traindata), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    valdata= "val.tsv"
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, valdata)),
        "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    dir_ = os.path.join(data_dir, "val.tsv")
    print(f'test file: {dir_}')
    return self._create_examples(
        self._read_tsv(dir_), "test")

  def get_labels(self):
    """See base class."""
    return ['agree', 'disagree']#,'unrelated','discuss']
...
def main(_):
  ...
  if FLAGS.do_predict_train:
    ...
    if not os.path.isfile(predict_file):
      predict_examples = processor.get_train_examples(FLAGS.data_dir)
      num_actual_predict_examples = len(predict_examples)
      if FLAGS.use_tpu:
        # TPU requires a fixed batch size for all batches, therefore the number
        # of examples must be a multiple of the batch size, or else examples
    ...
    output_predict_file = os.path.join(FLAGS.output_dir, "train_results.tsv")
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      num_written_lines = 0
      tf.logging.info("***** Predict results *****")
      for (i, prediction) in enumerate(result):
